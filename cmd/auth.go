...
$ curl -H "$(ideclare auth token -H --push --mount debian ubuntu)" ...

# To get the raw list tags response
$ curl -H "$(ideclare auth token -H ubuntu)" https://index.docker.io/v2/library/ubuntu/tags/list
`,
...
// NewCmdAuthGet creates a new `ideclare auth get` command.
...
// NewCmdAuthLogin creates a new `ideclare auth login` command.
...
// NewCmdAuthLogout creates a new `ideclare auth logout` command.

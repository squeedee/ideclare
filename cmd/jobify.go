...
// NewCmdJobify creates a new cobra.Command for the jobify subcommand.
func NewCmdJobify(options *[]crane.Option) *cobra.Command {
	var spec string
	...
	appendCmd := &cobra.Command{
		Use:   "jobify",
		Short: "Annotate an image with an ideclare job specification",
		Long: `This sub-command pushes an image based on an (optional)
base image, with an ideclare job specification attached.
...
container image.`,
	...
	appendCmd.Flags().StringVarP(&spec, "spec", "s", "-", "job spec file or '-' for stdin")
